...
ShellRoot {
    ...
    property var jsonfile: FileView {
        ...
        watchChanges: true
        blockLoading: true
        onFileChanged: colorTimer.restart()
    }

    Timer { interval: 3000; running: true; repeat: true; onTriggered: { jsonfile.reload(); root.parseColors(); } }
    Timer { id: colorTimer; interval: 200; onTriggered: { jsonfile.reload(); root.parseColors(); } }

    property var walColors: ({
        special: { background: "#1e1e2e", foreground: "#cdd6f4", cursor: "#cdd6f4" },
        colors: { 
            color0: "#1e1e2e", color1: "#f38ba8", color2: "#a6e3a1", color3: "#fab387", 
            color4: "#89b4fa", color5: "#cba6f7", color6: "#94e2d5", color7: "#bac2de",
            color8: "#585b70", color9: "#f38ba8", color10: "#a6e3a1", color11: "#fab387",
    ...
    function parseColors() {
        ...
        try {
            ...
            if (parsed && parsed.special && parsed.colors) {
                root.walColors = { special: parsed.special, colors: parsed.colors };
            }
        } catch (e) { console.log("Pywal: Error parsing colors.json:", e); }
    }

    Component.onCompleted: root.parseColors()

    // Mapping Catppuccin names to Pywal for backward compatibility in the UI
    property color base:     walColors.special.background
    property color text:     walColors.special.foreground
    property color surface0: walColors.colors.color8
    property color surface1: Qt.rgba(surface0.r, surface0.g, surface0.b, 0.8)
    property color surface2: Qt.rgba(surface0.r, surface0.g, surface0.b, 0.6)
    property color subtext0: walColors.colors.color15
    property color overlay2: walColors.colors.color7
    property color red:      walColors.colors.color1
    property color peach:    walColors.colors.color3
    property color mauve:    walColors.colors.color5
    ...
    WlSessionLock {
        ...
        WlSessionLockSurface {
            ...
            Item {
                ...
                property real validWidth: Screen.width > 100 ? Screen.width : 1920
                onWidthChanged: {
                    if (width > 100) {
                        validWidth = width;
                    }
                ...
                Scaler {
                    id: scaler
                    currentWidth: screenRoot.validWidth
                }
                readonly property real sc: scaler.baseScale
                // --------------------------------

                property string staticWallpaperPath: "file:///home/luisp/.cache/wallpaper.jpg"
                ...
                Process {
                    id: batPoller
                    command: ["bash", "-c", "if ls /sys/class/power_supply/ | grep -q '^BAT'; then echo 'true'; cat /sys/class/power_supply/BAT*/capacity 2>/dev/null | head -n1 || echo '100'; cat /sys/class/power_supply/BAT*/status 2>/dev/null | head -n1 || echo 'Unknown'; else echo 'false'; echo '100'; echo 'AC'; fi"]
                    stdout: StdioCollector {
                        onStreamFinished: {
                            let lines = this.text.trim().split("\n");
                            if (lines.length >= 3) {
                                screenRoot.hasBattery = lines[0] === "true";
